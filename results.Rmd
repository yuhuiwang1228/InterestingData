...
```{r}
#ggplot(dat_class_region, aes(x = Gross.Income.per.SqFt, y = #reorder(house_type, -Gross.Income.per.SqFt))) +
...
```{r}
dat_class_region_summary=dat_class_region%>%group_by(county)%>%count(house_type)%>%ungroup()
```

```{r}
#mosaic plot
vcd::mosaic(county ~ house_type, dat_class_region_summary)
```
